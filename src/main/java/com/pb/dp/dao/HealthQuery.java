...
public interface HealthQuery {
	...
	String GET_CUST_HEALTH = "select c.id as customerId,c.mobile,h.firstName,h.middleName,h.lastName,h.dob,mr.name as relationship,h.email " +
			"as emailId ,h.gender,h.id as healthdbId,h.healthId,h.healtIdNo,a2.line1 as address,a2.stateId,a2.districtId, ms.ndhm_name as state,md.ndhm_name as district," +
	...
	String GET_CUSTOMER_PROFILE = "select c.firstName,c.midName as middleName,c.lastName from customer c(nolock) where c.id=?";
	String GET_CUSTOMER_PROFILE_HEALTH = "select top 1 h.firstName,h.middleName,h.lastName from customer c(nolock) inner join healthId h(nolock) on h.customerId=c.id where c.id=? and h.isActive=1 ORDER by h.createdAt DESC";
}
