...
public interface HealthQuery {
	...
	String GET_CUST_HEALTH = "select c.id as customerId,c.mobile,h.firstName,h.middleName,h.lastName,h.dob,mr.name as relationship,h.email as emailId ,h.gender,h.id as healthdbId,h.healthId,h.healtIdNo,a2.line1 as address,ms.ndhm_name as state,md.ndhm_name as district,h.isKyc from customer c(nolock) inner join healthId h(nolock) on h.customerId=c.id inner join address a2 on a2.id=c.address_id inner join m_district md on md.ndhm_id = a2.districtId inner join m_state ms on ms.ndhm_id = a2.stateId inner join m_relation mr on mr.id = h.relation where c.id=? and c.mobile=? and h.isActive=1";
	...
	String GET_CUST_HEALTH_PROFILE = "select top 1 c.id as customerId,c.mobile,h.firstName,h.middleName,h.lastName,h.dob,mr.name as relationship,h.email as emailId ,h.gender,h.id as healthdbId,h.healthId,h.healtIdNo,a2.line1 as address,ms.ndhm_name as state,md.ndhm_name as district,h.isKyc,h.qrCode,h.healthCard from customer c(nolock) inner join healthId h(nolock) on h.customerId=c.id inner join address a2 on a2.id=c.address_id inner join m_district md on md.ndhm_id = a2.districtId inner join m_state ms on ms.ndhm_id = a2.stateId inner join m_relation mr on mr.id = h.relation where c.id=? and h.isActive=1 ORDER by h.createdAt DESC";
